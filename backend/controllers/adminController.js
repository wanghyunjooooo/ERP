...
exports.approveExpense = async (req, res) => {
    ...
    try {
        const result = await adminModel.updateExpenseApproval(expense_id, approved_by, approval_status);
        if (!result) return res.status(404).json({ error: "해당 지출 신청을 찾을 수 없습니다" });

        res.json({ message: "지출 승인 상태가 변경되었습니다", result });
    } catch (err) {
        console.error("지출 승인 오류:", err);
        res.status(500).json({ error: "지출 승인 실패" });
    }
};
